...
**AUTH-001 is `READY_FOR_RELEASE`** (human QA 2026-08-22; Issue
[#5](https://github.com/CrnkovicIvona/crm-ai-sdlc/issues/5)). Login UI
lives under `src/`. Copy `.env.example` to `.env` and add a non-prod
Supabase URL and anon key. Apply
`supabase/migrations/20260822150000_profiles.sql` to **non-prod** only.
Production merge is **human-only** (REL-003). CRM-001 is `SPECIFIED`
(not Ready). DASH-001 waits until CRM-001 is accepted for planning.
