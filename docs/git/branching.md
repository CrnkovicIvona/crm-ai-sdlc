...
- After that merge, the agent pushes the release sync to `test` (no
  second human PR unless the push is rejected). Squash releases will not
  share a SHA with `test`. Delete the release branch only after that
  sync is on `test`.
