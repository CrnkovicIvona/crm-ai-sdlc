...
| TC-D015  | AC-D015 | P    | decision  | e2e/int  | `tests/unit/clientLifecycleStamps.tc-d015.test.ts`; live `tests/integration/dashboard-stamps.test.ts` |
...
- Automation: `tests/unit/clientLifecycleStamps.tc-d015.test.ts`
  (always-on PGlite: VIEWER `clients` active-only, stamps match ADMIN
  New/Churned) and `tests/integration/dashboard-stamps.test.ts`
  `TC-D015` (live). SKIPPED live if secrets missing or view not on
  the target DB (not PASSED).
