...
| TC-D015  | AC-D015 | P    | decision  | e2e/int  | `tests/integration/dashboard-stamps.test.ts` `TC-D015`                |
...
- Expected: Same period, VIEWER New/Churned/Active/Rate/Net/Adoption
  ...
  PASSED.
- Automation: `tests/integration/dashboard-stamps.test.ts` `TC-D015`.
  SKIPPED if secrets missing or view not on the target DB (not PASSED).
