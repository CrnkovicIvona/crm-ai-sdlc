...
- Approval: BD-C001–BD-C018 approved; Gate 2 revised 2026-08-23
...
including soft-delete (FR-C003, FR-C016); optional Product catalog
assignments (FR-C014, FR-C015); VIEWER read and search (FR-C004);
full field visibility (FR-C005); database authorization boundary
(FR-C006); audit of successful CREATE/UPDATE/DELETE (FR-C007–FR-C010);
append-only audit (FR-C011); no READ or failed-attempt audit
(FR-C012, FR-C013).
...
| ID      | Statement                                                                                                                                                 | Source           |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| FR-C001 | CRM-001 has Client records, a fixed Product catalog, and optional ClientProduct assignments.                                                              | BD-C001, BD-C016 |
| FR-C002 | A Client has exactly these business fields: first name, last name, email, phone, OIB, created_at.                                                         | BD-C002          |
| FR-C003 | An authenticated ADMIN may READ, CREATE, UPDATE, and soft-DELETE Client records, and assign catalog products.                                             | BD-C003, BD-C018 |
...
| FR-C014 | CRM-001 includes a fixed Product catalog of six English banking products. There is no Product administration UI.                                          | BD-C016          |
| FR-C015 | A Client may have zero or more catalog products. Creating a Client must not require a product. ADMIN assigns products on create and edit.                 | BD-C017          |
| FR-C016 | UI DELETE soft-deletes the Client: the row leaves the active list, the row and audit remain, the audit action is DELETE. Restore is out of scope.         | BD-C018          |

## Business Rules

| ID      | Rule                                                             | Status   |
| ------- | ---------------------------------------------------------------- | -------- |
| BR-C001 | CRM-001 entities are Client, Product catalog, and ClientProduct. | Approved |
| BR-C009 | Products are optional on a Client.                               | Approved |
| BR-C010 | UI delete is soft-delete; no restore in CRM-001.                 | Approved |
...
- Product admin UI; restore; contract/rate/balance fields; extra Client fields; field-level security
...
| FR-C014 | BD-C016 | US-C007 | AC-C020         | Fixed Product catalog      | TC-C020         |
| FR-C015 | BD-C017 | US-C007 | AC-C021         | Optional product assign    | TC-C021         |
| FR-C016 | BD-C018 | US-C002 | AC-C022         | Soft-delete hides Client   | TC-C022         |

Technical design: [technical-spec.md](technical-spec.md).
Implementation: Client pages, `src/lib/clients.ts`, `src/lib/products.ts`,
and both CRM-001 migrations.
