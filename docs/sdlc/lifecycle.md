...
Business Request → Requirements → User Stories → Acceptance Criteria →
BDD/Gherkin → Test Cases → Test Plan → Traceability → Implementation
Plan → **Human Approval** → Feature Branch → Development → Unit Tests →
Integration Tests → Playwright/E2E Tests → Test Execution → Healing/Fix →
Regression → Test Report → Bug Management if required → Code Review →
Pull Request → `test` branch → CI/QA Gate → **Human Approval** →
Release Pull Request → `main` → Vercel Production → Post-deployment
Smoke Tests
...
| `REQUIREMENTS`      | Requirements being written                            | `analyze-requirements`                      | No                                           |
| `SPECIFIED`         | REQ, US, AC, BDD, test cases/plan, traceability exist | `plan-tests` complete                       | **Yes → `READY`** (DoR)                      |
