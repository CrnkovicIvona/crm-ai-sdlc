...
| `IN_DEVELOPMENT`    | Continue plan; add automated tests as specified (one TC → one `test()`/`it()`; see `plan-tests`). `ui-ux-redesign` may update the CRM-001 proposal; do not ship CSS until PO selects items                                                   | Claim TESTING without execution; group CRUD to hide which AC failed       |
...
| `IN_QA`             | Wait; if CI fails on the feature branch, `heal` from the job log (timeout vs assertion vs format). Optional `exploratory-qa-expert` if the human asked (Production; login-page test users; ADMIN+VIEWER; EXP report); do not self-approve QA | Self-approve QA gate                                                      |
| `READY_FOR_RELEASE` | `sync-feature-docs` at `ON_MAIN`; `release-and-verify` prepare only                                                                                                                                                                          | Merge `main`; claim `RELEASED`                                            |
| `ON_MAIN`           | Confirm Vercel Production. Production smoke workflow applies schema from `main` then smokes. If that **smoke** job PASSED: `sync-feature-docs` at `RELEASED` (PR to `main`)                                                                  | Claim `RELEASED` from merge or from apply-schema alone; merge `main`      |
