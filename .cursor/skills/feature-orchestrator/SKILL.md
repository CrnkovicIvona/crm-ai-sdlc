...
| `IN_QA`             | Wait; if CI fails on the feature branch, `heal` from the job log (timeout vs assertion vs format). Optional `exploratory-qa-expert` if the human asked; write EXP report; do not self-approve QA | Self-approve QA gate                                                      |
